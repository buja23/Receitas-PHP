...
if ($receita_id > 0) {
    if ($source === 'json') {
        ...
        $stmt = mysqli_prepare($conn, "SELECT * FROM receitas WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "i", $receita_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $db_receita = mysqli_fetch_assoc($result);

        if ($db_receita) {
            $receita_encontrada = [
                'titulo' => $db_receita['titulo'], 'thumbnail' => $db_receita['thumbnail'], 'descricao' => $db_receita['descricao'],
                'resumo' => ['tempo' => $db_receita['tempo'], 'pessoas' => $db_receita['pessoas'], 'nivel' => $db_receita['nivel']],
                'ingredientes' => !empty($db_receita['ingredientes']) ? explode("\n", str_replace("\r", "", $db_receita['ingredientes'])) : [],
                'modo_preparo' => !empty($db_receita['modo_preparo']) ? explode("\n", str_replace("\r", "", $db_receita['modo_preparo'])) : []
            ];
        }
        mysqli_close($conn);
    }
...
<body>
    <div class="container">
        <?php if ($receita_encontrada): ?>
            ...
            <?php if (!empty($receita_encontrada['ingredientes'])): ?>
            ...
            <?php if (!empty($receita_encontrada['modo_preparo'])): ?>
